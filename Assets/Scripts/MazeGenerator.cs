...
public class MazeGenerator
{
    ...
    public static string MAZEGEM = "G";
    ...
    public static string[,] CreateMaze(int p_mazesize, float p_complexity, int p_totalbigobject, int p_totalgem, bool p_hasstartpoint, bool p_hasfinishpoint) //this.exe <mazesize> <complexity> <totalbigobject>
    {
        int mazesize = 0;
        bool debug = false;
        if (debug)
        ...
        */
        ...
        for (int a = 0; a < p_totalgem; a++)
        {
            bool gemOK = false;
            while (!gemOK)
            {
                int ygem = rand.Next(0, maze.GetLength(0));
                int xgem = rand.Next(0, maze.GetLength(1));
                //Debug.Log(ygem.ToString() + " " + xgem.ToString());
                if (maze[ygem, xgem] == MAZEPATH)
                {
                    maze[ygem, xgem] = MAZEGEM;
                    gemOK = true;
                }
            }
        }
