...
public class CreateMazeMap : MonoBehaviour
{
    ...
    void Start()
    {
		...
		mapcube.transform.localScale = new Vector3(MazeDatabase.GetMaze[1].GetLength(0), MazeDatabase.GetMaze[1].GetLength(0), MazeDatabase.GetMaze[1].GetLength(0));
		for (int a = 1; a <= 6; a++)
		{
			...
			for (int y = 0; y < MazeDatabase.GetMaze[a].GetLength(0); y++)
			{
				for (int x = 0; x < MazeDatabase.GetMaze[a].GetLength(1); x++)
				{
					...
					else if (MazeDatabase.GetMaze[a][y, x] == "F")
					{
						...
						cube[a][y, x].transform.position = new Vector3(x - ((float)(MazeDatabase.GetMaze[a].GetLength(1)) / 2.0f) + 0.5f, 1f, y - ((float)(MazeDatabase.GetMaze[a].GetLength(0)) / 2.0f) + 0.5f);
						cube[a][y, x].transform.localScale = new Vector3(1, 1, 1);
						cube[a][y, x].transform.SetParent(plane[a].transform, true);
        ...
        cube[aa][yy, xx].transform.position = new Vector3(xx - ((float)(MazeDatabase.GetMaze[aa].GetLength(1)) / 2.0f) + 0.5f, 1f, yy - ((float)(MazeDatabase.GetMaze[aa].GetLength(0)) / 2.0f) + 0.5f);
		...
		//Bottom
		plane[1].transform.position = new Vector3(0, -0.5f * MazeDatabase.GetMaze[1].GetLength(0) +0.99f, 0);
		//Right
		plane[2].transform.position = new Vector3(0.5f * MazeDatabase.GetMaze[1].GetLength(0) - 0.99f, 0, 0);
		//Top
		plane[3].transform.position = new Vector3(0, 0.5f * MazeDatabase.GetMaze[1].GetLength(0) - 0.99f, 0);
		//Left
		plane[4].transform.position = new Vector3(-0.5f * MazeDatabase.GetMaze[1].GetLength(0) + 0.99f, 0, 0);
		//Front
		plane[5].transform.position = new Vector3(0, 0, 0.5f * MazeDatabase.GetMaze[1].GetLength(0)- 0.99f);
		//Back
		plane[6].transform.position = new Vector3(0, 0, -0.5f * MazeDatabase.GetMaze[1].GetLength(0) + 0.99f);
